...
UPLOAD_FOLDER = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'client', 'public', 'uploads')
...
def delete_marketplace_item(item_id):
    ...
    try:
        if item.image_path.startswith('/uploads/'):
            file_path = os.path.join(app.config['UPLOAD_FOLDER'], os.path.basename(item.image_path))
            if os.path.exists(file_path):
                os.remove(file_path)
...
def get_marketplace_item(item_id):
    ...
    if item.is_private and (not current_user.is_authenticated or item.author_id != current_user.id):
