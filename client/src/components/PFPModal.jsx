  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 m-4">
        <button onClick={handleClose} className="absolute right-4 top-4 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
          <X size={24} />
        </button>
        ...
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">Fetch a profile picture from GitHub or X!</p>
        </div>

        <div className="flex gap-2 mb-6">
          <button onClick={() => handlePlatformChange("github")} className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-medium transition-all duration-200 ${platform === "github" ? "bg-violet-500 text-white" : "bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300"}`}>
            <Github size={20} />
            <span>GitHub</span>
          </button>

          <button onClick={() => handlePlatformChange("x")} className={`flex-1 relative group flex items-center justify-center gap-2 py-3 rounded-xl font-medium transition-all duration-200 ${platform === "x" ? "bg-violet-500 text-white" : "bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300"}`}>
            ...
            {platform === "x" && (
              <div className="absolute inset-0 group-hover:bg-black/5 dark:group-hover:bg-white/5">
                <div className="absolute invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all duration-200 bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-2 bg-gray-900 text-white text-xs rounded-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="relative">
            <input ref={inputRef} type="text" value={username} onChange={(e) => setUsername(e.target.value)} placeholder={`Enter ${platform === "github" ? "GitHub" : "X"} username`} className="w-full px-4 py-3 bg-gray-100 dark:bg-gray-700 rounded-xl focus:ring-2 focus:ring-violet-500 focus:outline-none text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400" />
