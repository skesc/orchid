    ...
    <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div className="relative w-full max-w-md bg-neutral-200 rounded-2xl shadow-xl p-8 m-4">
        ...
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-violet-500 to-purple-500">
            Get Profile Picture
          </h2>
          <p className="mt-2 text-sm text-neutral-600 dark:text-neutral-400">
            Fetch a profile picture from GitHub or X!
          </p>
        </div>
        ...
        <div className="flex gap-2 mb-6">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-medium transition-all duration-200 ${platform === "github" ? "bg-violet-500 text-white" : "bg-neutral-300 text-neutral-600 "}`}>
            <Github size={20} />
            <span>GitHub</span>
          </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="relative">
            <input
              ...
              placeholder={`Enter ${platform === "github" ? "GitHub" : "X"} username`}
              className="w-full px-4 py-3 bg-neutral-300 rounded-xl focus:ring-2 focus:ring-violet-500 focus:outline-none text-neutral-900 placeholder-neutral-500 dark:placeholder-neutral-400"
            />
            <button
              type="submit"
              disabled={loading || !username.trim()}
              className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-violet-500 text-white rounded-lg hover:bg-violet-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200">
              {loading ? (
          ...
          {pfpUrl && (
            <div className="flex flex-col items-center gap-4">
              ...
              <button
                ...
                className="w-full py-3 bg-violet-500 text-white rounded-xl hover:bg-violet-600 transition-all duration-200">
                Use This Picture
              </button>
            </div>
          )}
