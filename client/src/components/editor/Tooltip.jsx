...
export const Tooltip = ({text, children, position = "right", shortcut}) => {
  ...
  return (
    <div className="relative flex items-center">
      ...
      <div className={`absolute z-50 px-2 py-1 text-sm font-medium text-white bg-neutral-900 rounded-md whitespace-nowrap ${positionClasses[position]} transition-opacity duration-200 ${show ? "opacity-100" : "opacity-0 pointer-events-none"}`} role="tooltip">
        <div className="flex items-center gap-2">
          <span>{text}</span>
          {shortcut && (
            <>
              <span className="text-neutral-400">|</span>
              <kbd className="px-1.5 py-0.5 text-xs font-semibold text-neutral-900 bg-neutral-200 rounded-md">{shortcut}</kbd>
            </>
          )}
        </div>
