  ...
  const handleDelete = async (itemUuid) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/admin/marketplace/${itemUuid}`,
        {
          method: "DELETE",
          credentials: "include",
        },
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-4 py-2 rounded-lg transition-colors ${filter === "all" ? "bg-violet-500 text-white" : "bg-white text-neutral-600 hover:bg-neutral-100"}`}>
            All
          </button>
          <button
            onClick={() => setFilter("public")}
            className={`px-4 py-2 rounded-lg transition-colors ${filter === "public" ? "bg-violet-500 text-white" : "bg-white text-neutral-600 hover:bg-neutral-100"}`}>
            Public
          </button>
          <button
            onClick={() => setFilter("private")}
            className={`px-4 py-2 rounded-lg transition-colors ${filter === "private" ? "bg-violet-500 text-white" : "bg-white text-neutral-600 hover:bg-neutral-100"}`}>
            Private
          </button>
        </div>
      </div>
              ...
              {item.categories.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {item.categories.map((category, index) => (
                    <span
                      key={index}
                      className="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-violet-50 text-violet-700">
                      {category}
                    </span>
                  ))}
                </div>
